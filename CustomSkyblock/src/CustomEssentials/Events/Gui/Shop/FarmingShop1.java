...
public class FarmingShop1 implements TabExecutor{

	private Main plugin;
	private ItemPrices prices;
	private ItemPrices pricesPrevious;
	
	public FarmingShop1(Main plugin, ItemPrices prices) {
		this.plugin = plugin;
		this.prices = prices;
		this.pricesPrevious = this.plugin.getShopPricesPrevious();
		plugin.getCommand("shopFarming1").setExecutor(this);
		
	}
	
	public Inventory createGui(Player p, Profile playerProfile) {
		
		Inventory menu = Bukkit.createInventory(null, 54,Utils.chat("&a&lFarming Shop &7(Page 1)"));
		...
		for (Material material : blocks.getFarming()) {
			if (i==17 || i== 26 || i == 35) {
				i += 2;
			}
			if (i > 43) break;
