...
public class BlockShop2 implements TabExecutor{

	private Main plugin;
	private ItemPrices prices;
	private ItemPrices pricesPrevious;
	
	public BlockShop2(Main plugin, ItemPrices prices) {
		this.plugin = plugin;
		this.prices = prices;
		this.pricesPrevious = this.plugin.getShopPricesPrevious();
		plugin.getCommand("shopBlocks2").setExecutor(this);
		
	}
	
	public Inventory createGui(Player p, Profile playerProfile) {
		
		Inventory menu = Bukkit.createInventory(null, 54,Utils.chat("&8&lBlocks Shop &7(Page 2)"));
		...
		for (int j = 28; j < blocks.getBlocks().size(); j++) {
