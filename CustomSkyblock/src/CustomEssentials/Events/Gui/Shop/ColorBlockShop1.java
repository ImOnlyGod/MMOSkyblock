...
public class ColorBlockShop1 implements TabExecutor{

	private Main plugin;
	private ItemPrices prices;
	
	public ColorBlockShop1(Main plugin, ItemPrices prices) {
		this.plugin = plugin;
		this.prices = prices;
		plugin.getCommand("shopColorBlocks1").setExecutor(this);
		
	}
	
	public Inventory createGui(Player p, Profile playerProfile) {
		
		Inventory menu = Bukkit.createInventory(null, 54,Utils.chat("&2&lColor Blocks Shop &7(Page 1)"));
		...
		for (Material material : blocks.getColorBlocks()) {
			if (i==17 || i== 26 || i == 35) {
				i += 2;
			}
			if (i > 43) break;
