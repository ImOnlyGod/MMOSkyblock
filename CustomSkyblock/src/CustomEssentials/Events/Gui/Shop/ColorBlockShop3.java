...
public class ColorBlockShop3 implements TabExecutor{

	private Main plugin;
	private ItemPrices prices;
	private ItemPrices pricesPrevious;
	
	public ColorBlockShop3(Main plugin, ItemPrices prices) {
		this.plugin = plugin;
		this.prices = prices;
		this.pricesPrevious = this.plugin.getShopPricesPrevious();
		plugin.getCommand("shopColorBlocks3").setExecutor(this);
		
	}
	
	public Inventory createGui(Player p, Profile playerProfile) {
		
		Inventory menu = Bukkit.createInventory(null, 54,Utils.chat("&2&lColor Blocks Shop &7(Page 3)"));
		...
		for (int j = 56; j < blocks.getColorBlocks().size(); j++) {
