...
public class MiscShop2 implements TabExecutor{

	private Main plugin;
	private ItemPrices prices;
	private ItemPrices pricesPrevious;
	
	public MiscShop2(Main plugin, ItemPrices prices) {
		this.plugin = plugin;
		this.prices = prices;
		this.pricesPrevious = this.plugin.getShopPricesPrevious();
		plugin.getCommand("shopMisc2").setExecutor(this);
		
	}
	
	public Inventory createGui(Player p, Profile playerProfile) {
		
		Inventory menu = Bukkit.createInventory(null, 54,Utils.chat("&f&lMiscellaneous Blocks Shop &7(Page 2)"));
		...
		for (int j = 28; j < blocks.getMisc().size(); j++) {
			Material material = blocks.getMisc().get(j);
			if (i==17 || i== 26 || i == 35) {
				i += 2;
			}
			if (i > 43) break;
