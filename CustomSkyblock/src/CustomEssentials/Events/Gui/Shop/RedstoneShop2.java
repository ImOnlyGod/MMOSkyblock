...
public class RedstoneShop2 implements TabExecutor{

	private Main plugin;
	
	public RedstoneShop2(Main plugin) {
		this.plugin = plugin;
	...
	public Inventory createGui(Player p, Profile playerProfile) {
		...
		Inventory menu = Bukkit.createInventory(null, 54,Utils.chat("&4&lRedstone Shop &7(Page 2)"));
		...
		for (int j = 28; j < blocks.getRedstone().size(); j++) {
			Material material = blocks.getRedstone().get(j);
			if (i==17 || i== 26 || i == 35) {
				i += 2;
			}
			if (i > 43) break;
